export const createCache = <T>({ expirationDelay }: { expirationDelay: number }) => {
  ...
  const getAll = (ids?: (string | symbol)[]) => {
    if (!ids) {
      return Object.values(CACHE).map((item) => item.value);
    }

    return ids.map((id) => CACHE[id]?.value).filter((item) => item !== undefined) as T[];
  };
  ...
  const hasAll = (keys: (string | symbol)[]) => {
    return keys.every((key) => CACHE[key] !== undefined);
  };

  const getMissingKeys = (keys: (string | symbol)[]) => {
    return keys.filter((key) => CACHE[key] === undefined);
  };
