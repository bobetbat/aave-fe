...
export class GoogleSheetsApiService {
  ...
  public async getSheet(sheetName: string): Promise<GoogleSheetsConnection | undefined> {
    ...
    const conn = await this.connectToGoogleSheets(this.sheetId, sheetName);

    this.sheets[key] = conn ?? ({} as GoogleSheetsConnection);
  ...
  public async connectToGoogleSheets(spreadsheetId: string, sheetTitle: string) {
    try {
      const credentials = {
        ...
        private_key: (process.env.GS_PRIVATE_KEY ?? '').replace(/\\n/g, '\n'), // Handle new lines in private_key
      };
